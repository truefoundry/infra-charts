...
| Name                                | Description                                                                                                                                                              | Value |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----- |
| `externalServices.prometheus.url`   | URL of an existing Prometheus instance. Must be set if prometheus.enabled=false. Example: http://`<release>`-prometheus-server.`<namespace>`.svc:9090 | `""`  |
...
| `prometheus.scrapeKubeStateMetrics`       | Scrape kube-state-metrics (required for control plane dashboard)                  | `true`                              |
| `prometheus.scrapeNodeExporter`           | Scrape node-exporter from the cluster (enable when reusing an existing DaemonSet) | `false`                             |
| `prometheus.kubeStateMetricsServiceRegex` | Regex for kube-state-metrics Service discovery                                    | `.*-kube-state-metrics`             |
| `prometheus.nodeExporterServiceRegex`     | Regex for node-exporter Service discovery                                         | `prometheus-prometheus-node-exporter` |
| `prometheus.server.retention`             | Data retention period                                                             | `15d`                               |
| `prometheus.server.retentionSize`         | Maximum storage size before oldest data is removed                                | `""`                                |
| `prometheus.server.resources`             | Resources for Prometheus server                                                   | `{}`                                |
| `prometheus.server.persistentVolume.size` | Persistent volume size                                                            | `50Gi`                              |
| `prometheus.server.affinity`              | Affinity for Prometheus pods                                                      | `{}`                                |
| `prometheus.server.tolerations`           | Tolerations for Prometheus pods                                                   | `[]`                                |
| `prometheus.server.nodeSelector`          | Node selector for Prometheus pods                                                 | `{}`                                |
...
| `headlamp.podSecurityContext`            | Pod security context                                                                                                                                                 | `{}`                |
| `headlamp.securityContext`               | Container security context                                                                                                                                           | `{}`                |
