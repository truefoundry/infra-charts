...
| `placement.podLabels`   | Labels applied to proxy and recorder pods.                                                                                                                                              | `{}`  |
| `placement.resources`   | Baseline resource requests and limits for Tailscale proxy and recorder containers. Each component below can override this; an override REPLACES this block rather than merging into it. | `{}`  |
...
| `proxyClasses.default.resources`        | Overrides placement.resources for the ingress/egress ProxyClass. REPLACES that block outright -- it is not merged, so restate requests as well as limits.                                                                  | `{}`                           |
...
| `proxyClasses.apiserver.resources`      | Overrides placement.resources for the kube-apiserver ProxyClass. REPLACES that block outright -- it is not merged, so restate requests as well as limits.                                                                  | `{}`                           |
...
### Tailnet admission policy

| Name                                          | Description                                                                                                                                                                                                                                                | Value                         |
| --------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------- |
| `tailnetAdmissionPolicy.enabled`              | Deny Tailscale-managed Services and Ingresses outside the derived namespace allowlist. Cluster-wide: it governs every namespace, not only this release's.                                                                                                  | `true`                        |
| `tailnetAdmissionPolicy.name`                 | Base name; each policy and binding is suffixed -services or -ingresses.                                                                                                                                                                                    | `tailnet-namespace-allowlist` |
| `tailnetAdmissionPolicy.additionalNamespaces` | Namespaces allowed on top of the derived set. For namespaces that own tailnet resources this chart does not render -- the derived list covers only ingresses.items, egress.items, tailnetServices.items, istioGateway.namespace and the release namespace. | `[]`                          |
...
| `istioGateway.selector`                  | Pod selector on the Gateway resource. Chooses which ingress pods program this listener.                                                                                                              | `{}`                    |
...
| `istioGateway.service.selector`          | Pods this Service sends to. Defaults to istioGateway.selector.                                                                                                                                       | `{}`                    |
| `istioGateway.service.hostname`          | Tailnet hostname for the gateway Service. Defaults to "<clusterSlug>-gw".                                                                                                                            | `""`                    |
| `istioGateway.service.allocateNodePorts` | Allocate a nodePort on every node for the gateway Service. Off: the proxy DNATs to the ClusterIP and never uses it.                                                                                  | `false`                 |
...
| `certificate.issuerRef`      | cert-manager issuer reference. Points at an existing issuer unless the issuer block above creates one. `kind` empty resolves to issuer.kind when issuer.enabled, and otherwise FAILS the render rather than guessing -- an issuer this chart did not create may be either kind, and naming the wrong one leaves the Certificate Pending forever. `group` is only for issuers outside cert-manager.io. | `{}`                                             |
...
| `certificates`               | Additional Certificates, keyed by resource name. Each needs secretName and dnsNames; namespace, issuerRef, duration and renewBefore default to the certificate block above.                                                                                                                                                                                                                           | `{}`                                             |
