...
{{/*
  Validate managed agents and Redis configuration.
*/}}
{{- define "tfy-llm-gateway.managedAgents.validate" -}}
{{- if and .Values.redis.enabled .Values.externalRedis.enabled -}}
{{- fail "redis.enabled and externalRedis.enabled cannot both be true" -}}
{{- end -}}
{{- if .Values.tags.managedAgents -}}
{{- if not .Values.gatewayId -}}
{{- fail "gatewayId is required when tags.managedAgents is true" -}}
{{- end -}}
{{- if not (or .Values.redis.enabled .Values.externalRedis.enabled) -}}
{{- fail "redis.enabled or externalRedis.enabled is required when tags.managedAgents is true" -}}
{{- end -}}
{{- end -}}
{{- end -}}

{{/*
  Resolve the LTS write job flag. An explicit value wins; otherwise it follows
  tags.managedAgents.
*/}}
{{- define "tfy-llm-gateway.agentsLtsWriteJob.enabled" -}}
{{- if hasKey .Values.agentsLtsWriteJob "enabled" -}}
{{- .Values.agentsLtsWriteJob.enabled -}}
{{- else -}}
{{- .Values.tags.managedAgents -}}
{{- end -}}
{{- end -}}
...
{{- include "tfy-llm-gateway.managedAgents.validate" . }}
...
- name: AGENT_SESSION_RESTORE_FROM_LTS_ENABLED
  value: {{ include "tfy-llm-gateway.agentsLtsWriteJob.enabled" . | quote }}
{{- end }}
