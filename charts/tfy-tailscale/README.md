...
over the upstream `tailscale-operator` chart: render-time validation of the values that fail
silently, and fleet-wide defaults.
...
This chart does not create the namespace — create it before installing. It needs no labels:
a sidecar's iptables fights the proxy DNAT, but injection is suppressed per-pod instead, via
`sidecar.istio.io/inject: "false"` on `tailscale-operator.operatorConfig.podLabels` here and
`placement.podLabels` in `tfy-tailscale-config`. Both default it on. That survives the
namespace being recreated, which a namespace label does not.
...
| `tailscale-operator.operatorConfig.podLabels`                | Labels applied to the operator pod. Defaults to suppressing Istio sidecar injection.                                                 | `{}`          |
