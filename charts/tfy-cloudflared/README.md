...
| `caddy.networkPolicy.enabled`            | Create a NetworkPolicy that only allows cloudflared pods (plus any `caddy.networkPolicy.allowedIngressFrom` peers) to reach the Caddy router on port 80. Requires a CNI that enforces NetworkPolicy; it is inert otherwise. Kubelet health probes originate from the node and are not blocked by this policy on common CNIs. | `true`                                |
