...
| `windowsVector.configMapName`          | Name of the ConfigMap containing Vector config to mount on Windows Vector pods. Defaults to the ConfigMap created by the victoria-logs-single subchart (`vector.customConfigName`, `vector-vl-config` in v0.11.x; was `vl-config` in older versions). | `vector-vl-config`                 |
